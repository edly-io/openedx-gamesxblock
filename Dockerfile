...
RUN mkdir -p /usr/local/src/openedx-gamesxblock

# Set the created directory as a Docker volume
VOLUME ["/usr/local/src/openedx-gamesxblock"]
...
RUN echo "pip install -r /usr/local/src/openedx-gamesxblock/requirements.txt" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "pip install -e /usr/local/src/openedx-gamesxblock" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
    echo "cd /usr/local/src/openedx-gamesxblock && make compile_translations && cd /usr/local/src/xblock-sdk" >> /usr/local/src/xblock-sdk/install_and_run_xblock.sh && \
